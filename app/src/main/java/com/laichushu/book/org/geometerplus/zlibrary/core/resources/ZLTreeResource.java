...
package com.laichushu.book.org.geometerplus.zlibrary.core.resources;
...
import com.laichushu.book.org.geometerplus.zlibrary.core.util.XmlUtil;
...
/**
 * ZLTreeResource类是ZLResource类的子类，ZLTreeResource类多了myChildren和myRoot属性。
 * ZLTreeResource类多出的两个属性可以用来表示母节点以及子节点。
 * 读取xml文件的三个核心类会配合ZLTreeResource类将xml资源文件转换成一个层级数据结构
 */
...
final class ZLTreeResource extends ZLResource {
	...
	private static void setInterfaceLanguage() {
		...
		if (Language.SYSTEM_CODE.equals(custom)) {
			...
			language = locale.getLanguage();//手机默认语言设置
			country = locale.getCountry();//手机默认的区域设置
		} else {
	...
	/**
	 * 最后，再补充下，默认的资源文件时uk.xml，
	 * 程序是怎么知道我们的手机应该是使用zh.xml的中文资源文件呢？
	 * 其实就是在ZLTreeResource类的updateLanguage方法。这个方法
	 */
			...
			if (!stack.isEmpty() && (NODE.equals(localName))) {
				...
				if (name != null) {
					...
					if (node == null) {
						...
						//相当与waitMessage节点代替了空的子节点，成为了根节点的子节点了
