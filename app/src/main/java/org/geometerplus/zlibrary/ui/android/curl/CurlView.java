...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
    ...
    public void onDrawFrame() {
        ...
        LoggerUtil.e("Time:" + currentTime);
        LoggerUtil.e("Time:" + mAnimationStartTime);
        LoggerUtil.e("Time:" + mAnimationDurationTime);
        // If animation is done.
        if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
            LoggerUtil.e("currentTime:");
...
//                if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
                    LoggerUtil.e("CurlRight-----mPageProvider.hasNextPage()");
...
//                if (mCurrentIndex < mPageProvider.getPageCount()) {
                ...
                if (mViewMode == SHOW_ONE_PAGE
                    ...
                    LoggerUtil.e("mViewMode = ShowOnePage");
                    mPageCurl.setRect(mRenderer
                            .getPageRect(CurlRenderer.PAGE_RIGHT));
                    mPageCurl.setFlipTexture(false);
                } else {
                    LoggerUtil.e("mViewMode = ShowTwoPage");
                    mPageCurl
                            .setRect(mRenderer.getPageRect(CurlRenderer.PAGE_LEFT));
    ...
    private void updatePages(ZLViewEnums.PageIndex pageIndex) {
        ...
        if(mCurlState == CURL_LEFT){    // 左边往右边翻
            LoggerUtil.e("mCurlState = CuRL_LEFT");
            pageIndex = pageIndex.getPrevious();
        }else if(mCurlState == CURL_RIGHT){ // 右边往左边翻
            pageIndex = pageIndex.getNext();
            LoggerUtil.e("mCurlState = CuRL_RIHGT");
        }else{
            LoggerUtil.e("mCurlState = CuRL_NONE");
        }

        switch (pageIndex){
            case previous:
                LoggerUtil.e("pageIndex = previous");
                if(!mPageProvider.hasPreviousPage()){   // 前一页不存在
                    LoggerUtil.e("previous---notHasPrevious");
                    updateRight(ZLViewEnums.PageIndex.current);
                }else{
                    LoggerUtil.e("previous---hasPrevious");
                    updateCurrent(ZLViewEnums.PageIndex.current);
                    updateLeft(ZLViewEnums.PageIndex.previous);
                }
                break;
            case next:
                LoggerUtil.e("pageIndex = next");
                if(!mPageProvider.hasNextPage()){
                    LoggerUtil.e("next---notHasNext");
                    updateRight(ZLViewEnums.PageIndex.current);
                }else{
                    LoggerUtil.e("next---hasNext");
                    updateCurrent(ZLViewEnums.PageIndex.current);
                    updateRight(ZLViewEnums.PageIndex.next);
                }
                break;
            case current:
                LoggerUtil.e("pageIndex = current");
                if(!mPageProvider.hasPreviousPage()){
                    LoggerUtil.e("current---notHasPrevious");
                    updateCurrent(pageIndex);
                    updateRight(pageIndex);
                }else{
                    LoggerUtil.e("current---hasPrevious");
                    updateRight(pageIndex);
                    updateLeft(ZLViewEnums.PageIndex.previous);
                }
                break;
