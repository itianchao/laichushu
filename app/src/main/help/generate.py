...
def generate_epub(proto_dir, html_dir, epub, lang):
    ...
    data = {
        'TITLE': title,
        'LANG':  lang,
        'ID':    'org.geometerplus.android.fbreader:intro:%s:%d' % (lang, time.time())
    }
    ...
    with zipfile.ZipFile(epub, 'w', zipfile.ZIP_DEFLATED) as zip_file:
        ...
        zip_file.write(os.path.join(proto_dir, 'org.geometerplus.android.fbreader.png'), arcname='org.geometerplus.android.fbreader.png')
